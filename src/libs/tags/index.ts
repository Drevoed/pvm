...
export function isGenericTagUsed(repoDir: string, config: Config): boolean {
  const { unified_versions_for, unified } = config.versioning
  const { release_tag_package } = config.tagging

  if (release_tag_package || !isMonorepo(repoDir) || unified) {
    return false
  }
...
export function isSemverTagUsed(repoDir: string, config: Config): boolean {
  return !isGenericTagUsed(repoDir, config)
}
...
export function releaseTagFilter(repoDir: string, config: Config): (string) => boolean {
  return isGenericTagUsed(repoDir, config) ? makeUnifiedReleaseTagTest(config) : isSemverTag
}

export function isReleaseTag(repoDir: string, config: Config, tag: string): boolean {
  return releaseTagFilter(repoDir, config)(tag)
}

export function releaseTagMask(repoDir: string, config: Config): string {
  const { prefix } = config.tagging.generic_tag

  return isGenericTagUsed(repoDir, config) ? `${prefix}-*` : semverTagMask
}
...
export function releaseTagPrefix(repoDir: string, config: Config): string {
  const { prefix } = config.tagging.generic_tag

  return isGenericTagUsed(repoDir, config) ? `${prefix}-` : 'v'
}
