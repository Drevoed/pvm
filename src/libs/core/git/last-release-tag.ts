...
export function lastReleaseTagIgnoreEnv(repoDir: string, config: Config, target: string | undefined = void 0): string {
  return lastMatchedTag(repoDir, releaseTagMask(repoDir, config), target)
}

export function lastReleaseTag(repoDir: string, config: Config, targetRef: string | undefined = void 0): string {
  const ciTag = env.CI_COMMIT_TAG
  const allowGetFromEnv =
    ciTag && (!targetRef || revParse(targetRef, repoDir) === revParse(ciTag, repoDir)) && isReleaseTag(repoDir, config, ciTag)

  return allowGetFromEnv ? ciTag! : lastReleaseTagIgnoreEnv(repoDir, config, targetRef)
}
...
export function prevReleaseTag(repoDir: string, config: Config, target = 'HEAD'): string {
  const latestReleaseTag = lastReleaseTag(repoDir, config, target)

  if (latestReleaseTag && revParse(latestReleaseTag, repoDir) === revParse(target, repoDir)) {
    return lastReleaseTag(repoDir, config, `${target}^`)
  }
