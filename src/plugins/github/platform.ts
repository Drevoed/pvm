...
import { PlatformInterface } from '@pvm/platform'
import type { IssueComment, PullRequest } from './types'

import { PlatformResult } from '@pvm/types-vcs'
...
export class GithubPlatform extends PlatformInterface<PullRequest> {
  ...
  constructor({ config, repoDir }: { config: Config, repoDir: string }) {
    super()
    this.config = config
    ...
    this.githubRepoPath = GithubPlatform.getRepoUrlParts(this.repoDir)
  ...
  async * releaseTagsIterator(): AsyncGenerator<PlatformReleaseTag, void, any> {
    const isReleaseTag = releaseTagFilter(this.repoDir, this.config)
  ...
  getCurrentBranch(): string | undefined {
    return env.GITHUB_HEAD_REF || env.GITHUB_REF_NAME || getCurrentBranchIgnoreEnv(this.repoDir)
  }
  ...
  private logReleaseTag(tagName: string): void {
    ...
    log.info(`release tag link: ${getHostUrl(this.repoDir)}/${CI_PROJECT_NAMESPACE}/${CI_PROJECT_NAME}/tags/${tagName}`)
  }
}
